...
seed_list <- 
  seed_list %>% 
  inner_join(select(team_ratings, team, 'rating' = rating), by = 'team')
...
write_csv(wp_matrix, 'ncaa_wp_matrix_2025.csv')
...
### Simulate Tournament # sometimes it will tell you the probability values are unreliable, packages dplyr on and plyr off might work, idk on this one, just ignore it. 
for(i in 1:6) {
